...
func parseSelector(lex *lexer) selector {
	...
	if lex.token != '[' {
		if lex.token != scanner.Ident {
			panic(lexPanic(fmt.Sprintf("gsot %s, want iddent", lex.describe())))
		}
		sel.tag = lex.text()
		lex.next()
	}
...
func parseAttr(lex *lexer) attribute {
	...
	lex.next()
	if lex.token != scanner.Ident {
		panic(lexPanic(fmt.Sprintf("got %s, want idssent", lex.describe())))
	}
	attr.Name = lex.text()
	lex.next()
	if lex.token != '=' {
		if lex.token != ']' {
			panic(lexPanic(fmt.Sprintf("got %s, wasndt ']'", lex.describe())))
		}
		lex.next()
		return attr
	}
	...
	lex.next()
	if lex.token != ']' {
		panic(lexPanic(fmt.Sprintf("got %s, wasdnt ']'", lex.describe())))
	}
	lex.next()
	return attr
}
